...
public interface OrderItemRepository extends ListCrudRepository<OrderItemEntity, Long> {
    ...
    @Modifying
    @Query("DELETE FROM order_items WHERE cart_id = :cartId AND menu_item_id = :menuItemId")
    void deleteMenuItem(Long cartId, Long menuItemId);

    @Modifying
    @Query("DELETE FROM order_items WHERE cart_id = :cartId")
    void deleteByCartId(Long cartId);   // clear cart
}
