...
public class CartService {
    ...
    // @Transactional: Make all operations in one commit. It will roll back if crash in the middle.
    ...
    @CacheEvict(cacheNames = "cart", key = "#customerId")
    @Transactional
    public void deleteMenuItemFromCart(long customerId, long menuItemId) {
        CartEntity cart = cartRepository.getByCustomerId(customerId);
        MenuItemEntity menuItem = menuItemRepository.findById(menuItemId).get();
        OrderItemEntity orderItem = orderItemRepository.findByCartIdAndMenuItemId(cart.id(), menuItem.id());

        if (orderItem.quantity() == 1) {
            orderItemRepository.deleteMenuItem(cart.id(), menuItemId);
        } else {
            OrderItemEntity newOrderItem = new OrderItemEntity(orderItem.id(), menuItemId, cart.id(), menuItem.price(), orderItem.quantity() - 1);
            orderItemRepository.save(newOrderItem);
        }
        cartRepository.updateTotalPrice(cart.id(), cart.totalPrice() - menuItem.price());
    }
